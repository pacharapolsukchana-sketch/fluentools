...
// Lazy load AdSpace component (not critical for initial render)
const AdSpace = dynamic(() => import('./components/common/AdSpace'), {
...
export default function Home() {
  ...
  const toggleFavorite = (toolId: string) => {
    ...
    if (mounted) localStorage.setItem('favorites', JSON.stringify(newFavorites))
  }

  const addToRecent = (toolId: string) => {
    const newRecent = [toolId, ...recent.filter(id => id !== toolId)].slice(0, 6)
    setRecent(newRecent)
    if (mounted) localStorage.setItem('recent', JSON.stringify(newRecent))
  }
  ...
  return (
    <MainLayout>
      ...
      <section className="relative bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 py-12 md:py-16 overflow-visible">
            ...
            <div className="relative max-w-xl mx-auto">
              ...
              {searchQuery && (
                <>
                  ...
                  <div 
                    className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-2xl border-2 border-gray-200 max-h-80 overflow-y-auto"
                    style={{ zIndex: 9999 }}
                    role="listbox"
                  >
      ...
      <section className="py-8 bg-gray-50">
        <div className="container mx-auto px-4" style={{ maxWidth: '80%' }}>
          ...
          <div className="flex items-center justify-center gap-2 mb-6 flex-wrap" role="tablist">
            {[
          ...
          {activeTab === 'popular' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" role="tabpanel" id="popular-panel">
                ...
                <div key={tool.id} className="relative group h-full">
                  <Link href={tool.href} onClick={() => addToRecent(tool.id)} className="block h-full">
                    <div className={`bg-gradient-to-br rounded-lg p-4 border-2 hover:shadow-lg transition-all h-full flex flex-col ${colorClasses[tool.color as keyof typeof colorClasses]}`}>
                      ...
                      <div className="text-[10px] text-gray-600 border-t border-gray-200/50 pt-2">Free</div>
                    </div>
                  </Link>
                  <button
          ...
          {activeTab === 'favorites' && (
            <div role="tabpanel" id="favorites-panel">
          ...
          {activeTab === 'recent' && (
            <div role="tabpanel" id="recent-panel">
